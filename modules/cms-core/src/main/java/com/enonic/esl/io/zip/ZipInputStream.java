...
/*
 * ZipInputStream.java -- Copyright (C) 2001, 2002, 2003, 2004 Free Software Foundation, Inc. This file is part of GNU
    ...
    implements ZipConstants
{
    ...
    private int readBuf( byte[] out, int offset, int length )
        throws IOException
    {
        if ( avail <= 0 )
        {
    ...
    /**
     * Open the next entry from the zip archive, and return its description. If the previous entry wasn't closed, this method will close
     * it.
     */
        ...
        throws IOException
    {
        ...
        try
        {
            name = getUTF8String( buffer, 0, nameLen );
        }
        catch ( IllegalArgumentException iae )
        {
            // NOTE! Hack!
            name = new String( buffer, "Cp850" );
        }
        ...
        throws IOException
    {
        if ( readLeInt() != EXTSIG )
        {
            throw new ZipException( "Data info signature not found" );
        }
                ...
                if ( len < 0 )
                {
                    ...
                    if ( inf.getTotalIn() != csize || inf.getTotalOut() != size )
                    {
                        throw new ZipException(
                            "size mismatch: " + csize + ";" + size + " <-> " + inf.getTotalIn() + ";" + inf.getTotalOut() );
                    }
                    inf.reset();
                    finished = true;
                }
