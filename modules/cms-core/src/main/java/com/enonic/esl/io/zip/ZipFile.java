...
/*
 * ZipFile.java -- Copyright (C) 2001, 2002, 2003, 2004, 2005 Free Software Foundation, Inc. This file is part of GNU
 * Classpath. GNU Classpath is free software; you can redistribute it and/or modify it under the terms of the GNU
 * General Public License as published by the Free Software Foundation; either version 2, or (at your option) any later
 * version. GNU Classpath is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the
 * implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more
 * details. You should have received a copy of the GNU General Public License along with GNU Classpath; see the file
 * COPYING. If not, write to the Free Software Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA.
    ...
    implements ZipConstants
{
     ...
     * @throws ZipException if the file doesn't contain a valid zip archive.
     */
    ...
    /**
     * Opens a Zip file reading the given File in the given mode. If the OPEN_DELETE mode is specified, the zip file will be deleted at some
     * time moment after it is opened. It will be deleted before the zip file is closed or the Virtual Machine exits. The contents of the
     * zip file will be accessible until it is closed.
     *
     ...
     */
    private int readLeShort( DataInput di, byte[] b )
        throws IOException
    {
        di.readFully( b, 0, 2 );
     ...
     * @throws EOFException if the file ends prematurely
     */
    private int readLeInt( DataInput di, byte[] b )
        throws IOException
    {
        di.readFully( b, 0, 4 );
    ...
    /**
     * Read the central directory of a zip file and fill the entries array. This is called exactly once when first needed. It is called
    ...
    {
        /*
         * Search for the End Of Central Directory. When a zip comment is present the directory may start earlier.
    ...
    /**
     * Closes the ZipFile. This also closes all input streams given by this class. After this is called, no further method should be
     * called.
     *
     * @throws IOException if a i/o error occured.
     */
     ...
     *
     * @throws IllegalStateException when the ZipFile has already been closed.
     * @throws IOEexception          when the entries could not be read.
     */
    private HashMap getEntries()
        throws IOException
    {
        synchronized ( raf )
        {
     ...
     *
     * @param the name. May contain directory components separated by slashes ('/').
     ...
     * @throws IOException  if a i/o error occured.
     * @throws ZipException if the local header doesn't match the central directory header
     */
    ...
    /**
     * Creates an input stream reading the given zip entry as uncompressed data. Normally zip entry should be an entry returned by
     * getEntry() or entries(). This implementation returns null if the requested entry does not exist. This decision is not obviously
     * correct, however, it does appear to mirror Sun's implementation, and it is consistant with their javadoc. On the other hand, the old
     * JCL book, 2nd Edition, claims that this should return a "non-null ZIP entry". We have chosen for now ignore the old book, as modern
     * versions of Ant (an important application) depend on this behaviour. See discussion in this thread:
