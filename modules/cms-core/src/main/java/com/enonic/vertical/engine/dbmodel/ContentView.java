    ...
    extends View
{
    ...
    public final static String STATE_SQL_CASE =
        "(case when cov_lStatus != 2 then cov_lStatus " + "when con_cov_lKey != cov_lKey then cov_lStatus " +
            "when con_dtePublishFrom is null and con_dtePublishTo is null then 2 " +
            "when con_dtePublishFrom is null and con_dtePublishTo <= @currentTimestamp@ then 6 " +
            "when con_dtePublishFrom is null and con_dtePublishTo > @currentTimestamp@ then 2 " +
            "when con_dtePublishFrom > @currentTimestamp@ then 4 " +
            "when con_dtePublishFrom <= @currentTimestamp@ and ( con_dtePublishTo is null OR con_dtePublishTo > @currentTimestamp@ ) then 5 " +
            "when con_dtePublishTo <= @currentTimestamp@ then 6 " + "else 0 end)";
    ...
    private final static String SQL = "select con_lKey, cov_lKey, cat_lKey, cat_uni_lKey, cat_cty_lKey, " +
        ...
        "o.usr_sFullName as usr_sOwnerName, o.usr_bIsDeleted as usr_bOwnerDeleted, " + "con_dteCreated, cov_lStatus, " + STATE_SQL_CASE +
        " as cov_lState, " + "con_dtePublishFrom, con_dtePublishTo, cov_sTitle, cov_xmlContentData, con_lPriority, " +
        "cov_usr_hModifier as usr_hModifier, m.usr_sUID as usr_sModifierUID, m.usr_sFullName as usr_sModifierName, " +
        "m.usr_bIsDeleted as usr_bModDeleted, cov_dteTimestamp " + "from tContent left join tContentVersion on con_cov_lKey = cov_lKey " +
    ...
    private static final ContentView Content = new ContentView( "vContent" );

    private ContentView( String tableName )
    {
        super( tableName, "content", "contents", SQL, 9 );
        addColumn( con_lKey );
        addColumn( cov_lKey );
        addColumn( cat_lKey );
