                            ...
                            <table width="100%" cellspacing="0" cellpadding="0" class="browsetable">
                                <tr>
                                    <xsl:call-template name="tablecolumnheader">
                                        <xsl:with-param name="caption" select="'%fldHostName%'" />
                                        ...
                                        <xsl:with-param name="sortby" select="'server/hostname'" />
                                    </xsl:call-template>

                                    <xsl:call-template name="tablecolumnheader">
                                        <xsl:with-param name="width" select="'80'"/>
                                        <xsl:with-param name="align" select="'center'"/>
                                        <xsl:with-param name="caption" select="'%fldPort%'" />
                                        <xsl:with-param name="pageURL" select="$pageURL" />
                                        <xsl:with-param name="current-sortby" select="$sortby" />
                                        <xsl:with-param name="current-sortby-direction" select="$sortby-direction" />
                                        <xsl:with-param name="sortby" select="'server/port'" />
                                    </xsl:call-template>
                                ...
                                <xsl:variable name="sortby-data-type">
                                    <xsl:choose>
                                        <xsl:when test="$sortby = 'server/port'">number</xsl:when>
                                        <xsl:otherwise>text</xsl:otherwise>
                                    </xsl:choose>
                                </xsl:variable>

                                <xsl:for-each select="/ldapservers/ldapserver">
                                    <xsl:sort data-type="{$sortby-data-type}" order="{$sortby-direction}"
                                        select="*[name() = $sortby] | @*[concat('@',name()) = $sortby] | *[name() = substring-before($sortby, '/')]/*[name() = substring-after($sortby, '/')]"/>
                                    ...
                                    <tr>
                                        ...
                                        <td class="browsetablecell" title="%msgClickToEdit%">
                                            ...
                                            <xsl:value-of select="server/hostname"/>
                                        </td>
